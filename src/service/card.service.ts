...
import { CreateStaffDto } from '../dto/microengine.dto'
import {
	addTimeToDateString,
...
export class CardService {
	...
	createQrCodeService = async (userGuid: string, role: keyof typeof RoleEnum) => {
		try {
			...
			await this.createMicroEngineUser(role, userData.userId, userData.badgeNumber, {
				...StaffConst,
				Card: {
					...StaffConst.Card,
	...
	createUpdateVisitorFaceAuthService = async (
		...
		try {
			...
			if (
				!isWithinTimeRange(convertTimestampToUserTimezone(visitorDetails.visitDateTime, ITimeFormat.isoDateTime), 15)
			) {
			...
			if (badgeNumber === '') {
				const result = await this.createBadgeNumber(staffGuid, RoleEnum.VI, createUpdateVisitorFaceAuthDto.visitorGuid)
				if (result === null) {
					throw new OperationError('Failed to create badge number', HttpStatusCode.INTERNAL_SERVER_ERROR)
				}
				badgeNumber = result.toString()
				await this.visitorRepository.editVisitorByIdRepository(createUpdateVisitorFaceAuthDto.visitorGuid, {
			...
			await this.createMicroEngineUser('VI', visitorDetails.id, badgeNumber, {
				...StaffConst,
				Profile: {
					...StaffConst.Profile,
	...
	private async createMicroEngineUser(
		...
		const userCard = await this.microEngineService.getUserById(this.generateUserId(role, userId, staffInfo.Card.BadgeCategory))
		if (!userCard) {
			await this.microEngineService.addUser(staffInfo, badgeNumber)
