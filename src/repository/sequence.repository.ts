...
import { Sequences } from '../model/sequences.model'
...
export class SequenceRepository {
	...
	async getSequenceId(args: incrementParams) {
		...
		const counterRef = this.firebaseAdmin.firestore.doc(`sequences/${args.counterName}`)
		const counterDoc = await args.transaction.get(counterRef)
		if (counterDoc.exists) {
			const { sequenceValue } = counterDoc.data() as Sequences
